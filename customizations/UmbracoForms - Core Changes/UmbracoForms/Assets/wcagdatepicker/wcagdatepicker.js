function datepicker(id, target, modal, calid, bnprevid, bnnextid, monthid) {

   this.$instanceId = id; 
   this.$id = $('#' + id); // element to attach widget to
   this.$monthObj = $('#' + monthid);
   this.$prev = $('#' + bnprevid); 
   this.$next = $('#' + bnnextid); 
   this.$grid = $('#' + calid); 
...
datepicker.prototype.popGrid = function() {
   ...
   var gridCells = '\t<tr id="row1'+this.$instanceId+'">\n';
   ...
   for (curDay = 1; curDay <= numDays; curDay++) {
      ...
      if (curDay == this.date && this.currentDate == true) {
         ...
         gridCells += '\t\t<td id="day' + curDay + this.$instanceId + '" class="today" headers="row' +
                      rowCount + ' ' + this.dayNames[weekday] + '" role="gridcell" aria-selected="false">' + curDay + '</td>';

      }
      else {
         gridCells += '\t\t<td id="day' + curDay + this.$instanceId + '" headers="row' +
                      rowCount + ' ' + this.dayNames[weekday] + '" role="gridcell" aria-selected="false">' + curDay + '</td>';
      }
      ...
      if (weekday == 6 && curDay < numDays) {
         ...
         // and begin a new one
         gridCells += '\t</tr>\n\t<tr id="row' + rowCount + this.$instanceId + '">\n';
         rowCount++;
         weekday = 0;
      }
      else {
...
datepicker.prototype.showPrevMonth = function(offset) {
   ...
   if (offset != null) {
      var numDays = this.calcNumDays(this.year, this.month);
      var day = 'day' + (numDays - offset) + this.$instanceId;
      
      this.$grid.attr('aria-activedescendant', day);
      $('#' + day + this.$instanceId).addClass('focus').attr('aria-selected', 'true');
   }
...
datepicker.prototype.showNextMonth = function(offset) {
   ...
   this.$monthObj.html(this.monthNames[this.month] + ' ' + this.year);
      ...
      if (offset != null) {
         var day = 'day' + offset + this.$instanceId;
...
datepicker.prototype.handlePrevClick = function(e) {
   ...
   if (this.currentDate == false) {
      this.$grid.attr('aria-activedescendant', 'day1' + this.$instanceId);
   }
   else {
      ...
      switch(e.keyCode) {
         ...
         case this.keys.pageup: {
            ...
            if ($('#' + active).attr('id') == undefined) {
               var lastDay = 'day' + this.calcNumDays(this.year, this.month) + this.$instanceId;
